...
public class ImmediateModeShapeRenderer extends BaseShapeRenderer
{
    ...
    public void fillPartialOval(double x, double y, double sX, double sY, double start, double end)
    {
        ...
        glBegin(GL_TRIANGLES);
        double step = (end - start) / sides;
        for (double i = start; i < end - 0.0001; i += step)
        {
            glVertex2d(x + Math.cos(i) * sX / 2, y + Math.sin(i) * sY / 2);
            glVertex2d(x + Math.cos(i + step) * sX / 2, y + Math.sin(i + step) * sY / 2);
            glVertex2d(x, y);
        }
    ...
    public void drawGradientImage(double x, double y, double sX, double sY, double u1, double v1, double u2, double v2, double a1, double a2, double a3, double a4, String image, boolean scaled)
    {
        if (this.window.drawingShadow)
            return;
        ...
        glColor4d(window.colorR, window.colorG, window.colorB, a1);
        glTexCoord2d(u1, v1);
        glVertex2d(x, y);

        glColor4d(window.colorR, window.colorG, window.colorB, a2);
        glTexCoord2d(u1, v2);
        glVertex2d(x, y + height);

        glColor4d(window.colorR, window.colorG, window.colorB, a3);
        glTexCoord2d(u2, v2);
        glVertex2d(x + width, y + height);

        glColor4d(window.colorR, window.colorG, window.colorB, a4);
        glTexCoord2d(u2, v1);
        glVertex2d(x + width, y);
    ...
    protected void setUpscaleMode()
    {
        if (this.window.upscaleImages)
        {
